...
def load_face_keypoints(df, frame_idx):
    """从CSV加载面部关键点"""
    prefix = 'face_'
    count = 468  # MediaPipe Face Mesh
    pts = np.full((count, 2), np.nan, dtype=float)
    for i in range(count):
        cx = f'{prefix}{i}_x'
        cy = f'{prefix}{i}_y'
        if cx in df.columns and cy in df.columns:
            pts[i, 0] = df[cx].iloc[frame_idx]
            pts[i, 1] = df[cy].iloc[frame_idx]
    return pts
...
def warp_face_triangles(src_img, src_pts, tgt_pts, triangles):
    ...
    for tri in triangles:
        if len(tri) == 3:
            src_tri = src_pts[tri]
            tgt_tri = tgt_pts[tri]
            if np.any(np.isnan(src_tri)) or np.any(np.isnan(tgt_tri)):
                continue
            # 计算仿射变换
            M = cv2.getAffineTransform(src_tri.astype(np.float32), tgt_tri.astype(np.float32))
            # 应用变换到三角形区域
            mask = np.zeros_like(src_img[:, :, 0])
            cv2.fillConvexPoly(mask, tgt_tri.astype(np.int32), 255)
            warped_tri = cv2.warpAffine(src_img, M, (src_img.shape[1], src_img.shape[0]), borderMode=cv2.BORDER_REFLECT)
            warped[mask > 0] = warped_tri[mask > 0]
    return warped
...
def main():
    ...
    # 加载关键点
    src_pts = load_face_keypoints(df, 0)  # 帧0作为源
    tgt_pts = load_face_keypoints(df, 1)  # 帧1作为目标

    # 获取三角形
    triangles = get_face_triangles()

    # 变形
    warped = warp_face_triangles(src_img, src_pts, tgt_pts, triangles[:100])  # 只用前100个三角形测试

    # 保存结果
    cv2.imwrite('face_warped_test.jpg', warped)
    print("变形结果保存为face_warped_test.jpg")
